...
export function CollectionSwitcher() {
  ...
  const handleToggle = (tier: "all" | "premium" | "simple") => {
    if (priceTier === tier && tier !== "all") {
    ...
    <div className="flex items-center justify-center border-b border-line bg-background/80 py-1.5 text-xs backdrop-blur-xl">
      <div className="flex rounded-full border border-line bg-panel p-1 shadow-sm">
        <button
          ...
          className={`flex items-center gap-1.5 rounded-full px-4 py-1.5 font-medium tracking-wide uppercase transition duration-300 ${
            priceTier === "all"
        ...
        >
          <Sparkles size={12} className={priceTier === "premium" ? "text-white" : ""} />
